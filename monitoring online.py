...
# Backend URL
FLASK_URL = 'https://fe09-182-1-98-111.ngrok-free.app/'
UNDERWATER_URL = (f"{FLASK_URL}images/ubox_1.jpg")
SURFACE_URL = (f"{FLASK_URL}images/sbox_1.jpg")

# https://5105-103-53-76-210.ngrok-free.app
def data_backend():
    try:
        response = requests.get(f"{FLASK_URL}data-receive")
...
def surface_backend():
    try:
        response = requests.get(SURFACE_URL)
        response.raise_for_status()
        return response.content
    except requests.RequestException:
        return None    
def underwater_backend():
    try:
        response = requests.get(UNDERWATER_URL)
        response.raise_for_status()
        return response.content
    except requests.RequestException:
        return None
...
def hasil_foto_surface(konten_gambar1):
    with image_placeholder.container():
        col1, _ = st.columns([1, 1])
        with col1:
            st.markdown("<h6 class='judul-text'>Surface Picture</h6>", unsafe_allow_html=True)
            st.image(konten_gambar1)  # Menggunakan konten gambar yang diterima
        
def hasil_foto_underwater(konten_gambar2):
    with image_placeholder.container():
        _, col2 = st.columns([1, 1])
        with col2:
            st.markdown("<h6 class='judul-text'>Under Water Picture</h6>", unsafe_allow_html=True)
            st.image(konten_gambar2)
    
def update_gambar():
    konten_gambar1 = surface_backend()
    konten_gambar2 = underwater_backend()
    if konten_gambar1:
        hasil_foto_surface(konten_gambar1)  # Mengirim konten gambar
    if konten_gambar2:
        hasil_foto_underwater(konten_gambar2)
